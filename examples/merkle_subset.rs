...
pub fn zero_hash<F: RichField, H: Hasher<F>>() -> H::Hash {
...
pub fn ground_proof<F: RichField + Extendable<D>, C: GenericConfig<D, F = F>, const D: usize>(
    inp1: &Vec<F>,
    inp2: &Vec<F>,
) -> ProofTuple<F, C, D> {
...
{
    let mut ground_size: usize;
    if height > 1 {
        ground_size = trivial_proofs.len() as usize;
        ground_size = ground_size / 2;
        let sec = ground_size as u64;
        let first_tuple = recursive_tree::<F, C, D>(
            location,
            height - 1,
            &trivial_proofs[0..ground_size].to_vec(),
        );
        let second_tuple = recursive_tree::<F, C, D>(
            location + sec,
            height - 1,
            &trivial_proofs[ground_size..ground_size * 2].to_vec(),
        );
    ...
    C::Hasher: AlgebraicHasher<F>,
{
    ...
    let num_leaves: usize = inputs1.len();
    ...
    trivial_proofs.push(ground_proof::<F, C, D>(&inputs1[0], &inputs2[0]));
    for i in 1..num_leaves {
        trivial_proofs.push(ground_proof::<F, C, D>(&inputs1[i], &inputs2[i]));
    }
    println!("Lets come to final proof!");
    let final_proof = recursive_tree::<F, C, D>(0, depth, &trivial_proofs);
    println!(
        "Final proofs public inputs: {:#?}",
    ...
    println!(
        "Allegedly, the result of our poseidon is: {:#?}",
        final_proof.proof.public_inputs,
    );
    ...
    (
        Ok(VerifierCircuitData::<F, C, D> {
            verifier_only: final_proof.vd,
            common: final_proof.cd,
        }),
...
pub fn roots<F: RichField, H: Hasher<F>>(merkle_cap: &MerkleCap<F, H>) -> H::Hash {
...
pub fn test() -> Result<()> {
    ...
    let mut original_leaves: Vec<Vec<F>> = Vec::new();
    for i in 0..256 {
        let mut tmp = Vec::new();
        for j in 0..4 {
            if i == j {
    ...
    //     [F::ZERO, F::ZERO, F::ZERO, F::ONE].to_vec(),
    // ]
    // .to_vec();
    ...
    let (vd, pb1, pb2, proof) = run::<F, C, H, D>(original_leaves.clone(), subset_leaves.clone());
    let vd = vd.unwrap();

    let compressed_proof_with_pis = proof
        .compress(&vd.verifier_only.circuit_digest, &vd.common)
        .unwrap();
    println!(
        "Compressed proof size: {} bytes\n",
        compressed_proof_with_pis.to_bytes().len()
    );

    let result = vd.verify_compressed(compressed_proof_with_pis);
    assert!(result.is_ok());
    ...
    let original_merkle_tree: MerkleTree<GoldilocksField, PoseidonHash> =
        MerkleTree::<F, H>::new(original_leaves.clone(), cap_height);
    let subset_merkle_tree: MerkleTree<GoldilocksField, PoseidonHash> =
        MerkleTree::<F, H>::new(subset_leaves.clone(), cap_height);

    if roots(&original_merkle_tree.cap).elements == pb1 {
        println!("Original merkle roots matches");
    }
    if roots(&subset_merkle_tree.cap).elements == pb2 {
